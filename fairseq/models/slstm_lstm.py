    ...
    def extract_features(
        ...
        **unused,
        ...
        for i, layer in enumerate(self.layers):
            ...
            x, attn = layer(
                x,
                encoder_out=encoder_out["all_encoder_out"][i]
                if self.layer_wise_attention
                else encoder_out["encoder_out"][0],  # use layer-wise attn or not
                src_padding_mask=encoder_out["encoder_padding_mask"],
...
class LSTMTransformerDecoderLayer(nn.Module):
    ...
    def _get_input_buffer(self, incremental_state: dict) -> BufferType:
        return fairseq_utils.get_incremental_state(
            self, incremental_state=incremental_state, key="rnn_hidden_state"
        )

    def _set_input_buffer(self, incremental_state: dict, buffer: BufferType):
        return fairseq_utils.set_incremental_state(
            self, incremental_state=incremental_state, key="rnn_hidden_state", value=buffer
        )

    def reorder_incremental_state(self, incremental_state: dict, new_order: torch.Tensor):
        ...
        h, c = buffer
        new_buffer = (h.index_select(1, new_order), c.index_select(1, new_order))
    ...
    def forward(
        ...
        if incremental_state:
            ...
            if not prev_states:
                prev_states = (
                    init_states[0].unsqueeze(0),
                    init_states[1].unsqueeze(0),
                )  # set init states
...
@register_model_architecture("slstm_tlstm", "slstm_tlstm")
...
@register_model_architecture("slstm_tlstm", "slstm_tlstm_base")
def slstm_tlstm_base(args):
    base_architecture(args)


@register_model_architecture("slstm_tlstm", "slstm_tlstm_iwslt")
