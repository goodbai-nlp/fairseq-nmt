...
from fairseq.modules.vslstm_feat_fc4 import VanillaSLSTMFeat
from fairseq.modules.vslstm_dec_feat_fc4 import VanillaSLSTMFeatDecLayer
from torch import Tensor
BufferType = Tuple[torch.Tensor, torch.Tensor]
DEFAULT_MAX_SOURCE_POSITIONS = 1024


@register_model("vslstm")
class VanillaSlstmModel(FairseqEncoderDecoderModel):
...
class SLSTMEncoder(FairseqEncoder):
    ...
    def reorder_encoder_out(self, encoder_out, new_order):
        ...
        if encoder_out["encoder_padding_mask"] is not None:
            ...
            # print(encoder_out["encoder_padding_mask"][0].size())
            # print(encoder_out["encoder_padding_mask"][1].size())
            # xx
            # print(encoder_out["encoder_padding_mask"][0].size())
            encoder_out["encoder_padding_mask"] = [encoder_out["encoder_padding_mask"][0].index_select(
                0, new_order
            )]
            # print(encoder_out["encoder_padding_mask"].size())
            # xx
...
class SLSTMDecoder(FairseqIncrementalDecoder):
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.layer = self.build_decoder_layer(args, no_encoder_attn)
    ...
    def extract_features_scriptable(
        ...
        we_cat = self.layer.emb_gate_linear(x.view(-1, hidden_size))
        hidden_states = x
        cell_states = x
        for idx in range(self.num_layers):
            ...
            # xx
            hidden_states, cell_states = self.layer(hidden_states, cell_states, we_cat, self_attn_mask=self_attn_mask, encoder_out=encoder_out, incremental_state=incremental_state, seq_length=None)
            ...
            # )
            inner_states.append(hidden_states)
        ...
        return hidden_states, {"attn": [attn], "inner_states": inner_states}

    def build_decoder_layer(self, args, no_encoder_attn=False):
        layer = VanillaSLSTMFeatDecLayer(args, no_encoder_attn)
...
@register_model_architecture("vslstm", "vslstm")
