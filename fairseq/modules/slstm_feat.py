...
class SLSTMFeat(nn.Module):
    def __init__(self, args):
        # current
        super(SLSTMFeat, self).__init__()
    ...
    def forward(self, word_inputs, mask, num_layers, seq_length=None):
        ...
        hidden_f0, cell_f0 = self.attn_feat_extractor(initial_hidden_states, bool_mask, None) 
        ...
        for i in range(num_steps):
            ...
            cat_gate_value = (
                self.norm_gate(
                    torch.cat([initial_hidden_states, concat_before_after, hidden_f0], dim=1)
                )
                + we_cat
            )
            ...
            prev_state = (hidden_f0, cell_f0)
            hidden_f0, cell_f0 = self.attn_feat_extractor(initial_hidden_states, bool_mask, prev_state) 
